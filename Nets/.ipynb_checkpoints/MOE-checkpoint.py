...
#     """
#     像素级空间门控网络，用于MoE中的专家选择
    
#     功能:
#         - 在每个像素点上为多个专家生成权重
#         - 通过温度参数控制门控的平滑度
#         - 保持输入的空间分辨率
#         - [优化] 最后一层零初始化，保证训练初期各专家概率均等
    
#     Args:
#         input_size (int): 输入通道数
#         num_experts (int): 专家数量
#         temperature (float): Softmax温度参数，默认1.0
    
#     Input:
#         x: [B, C, H, W] - 输入特征图
    
#     Output:
#         gate_weights: [B, num_experts, H, W] - 每个像素、每个专家的权重
#     """
    
#     def __init__(self, input_size, num_experts, temperature=1.0):
#         super(SpatialGateNetwork, self).__init__()
#         self.input_size = input_size
#         self.num_experts = num_experts
        
#         # 全卷积网络提取门控逻辑
#         # 结构: Conv(3x3) -> LeakyReLU -> Conv(1x1)
#         self.gate_conv = nn.Sequential(
#             nn.Conv2d(input_size, input_size // 2, kernel_size=3, padding=1),
#             nn.LeakyReLU(0.2, inplace=True),
#             nn.Conv2d(input_size // 2, num_experts, kernel_size=1)
#         )
        
#         # 注册温度参数 (作为buffer，随模型保存但不作为参数更新)
#         self.register_buffer('temperature', torch.tensor(temperature, dtype=torch.float32))
#         self.softmax = nn.Softmax(dim=1)

#         # 【核心优化】初始化最后一层卷积的权重和偏置为0
#         # 效果：初始 logits 全为 0 -> Softmax 后概率均匀分布(1/N)
#         # 目的：防止训练初期因随机初始化导致模型偏向某一个专家(Expert Collapse)
#         nn.init.constant_(self.gate_conv[-1].weight, 0)
#         nn.init.constant_(self.gate_conv[-1].bias, 0)
...
#         Returns:
#             gate_weights: [B, num_experts, H, W] 门控权重
#         """
#         # 生成门控logits: [B, num_experts, H, W]
#         logits = self.gate_conv(x)
        
#         # 应用温度缩放
#         # 使用 clamp 防止温度过低导致数值不稳定(除零保护)
#         temp = torch.clamp(self.temperature, min=1e-3)
        
#         # 计算 Softmax，维度1是专家维度
#         gate_weights = self.softmax(logits / temp)
...
#         Args:
#             new_temperature (float): 新的温度值
#         """
#         self.temperature.fill_(new_temperature)
    
#     def get_expert_utilization(self, gate_weights):
#         """
#         计算专家利用率（用于监测负载均衡）
...
#         Returns:
#             utilization: [num_experts] 每个专家的平均使用率
#         """
#         # 计算每个专家在batch中的平均权重
#         # 对 Batch(0), Height(2), Width(3) 维度求平均，保留 Expert(1) 维度
#         utilization = gate_weights.mean(dim=(0, 2, 3))  # [num_experts]
#         return utilization
...
#     """
#     Spatial-Aware Attribute-aware Mixture of Fusion Experts (v2)
#     集成空间门控和改进的专家模块
#     """
#     def __init__(self, channels, num_experts=3, temperature=1.0):
#         super(AMoFE, self).__init__()
        
#         # 使用新的空间门控网络
#         self.gate = SpatialGateNetwork(channels, num_experts, temperature)
 
#         # 初始化改进后的专家列表
#         self.expert_networks_d = nn.ModuleList([
#             SpatialExpert(channels),          # Expert 0: 基础空间特征
#             RealSpectralExpert(channels),     # Expert 1: FFT 全局频域特征
#             OptimizedEdgeExpert(channels),    # Expert 2: 优化边缘特征
#         ])
...
#             # 像素级加权累加
#             # [B, C, H, W] * [B, 1, H, W] -> Broadcast multiply
...
class ContextAwareNoisySpatialGate(nn.Module):
    """
    上下文感知噪声空间门控 (Context-Aware Noisy Spatial Gate)
    
    核心特性:
    1. 双流感知: 结合了全局上下文(Global Context)和局部空间特征(Local Spatial Features)。
    2. 噪声机制: 继承了原代码的噪声生成与归一化逻辑，但在像素级进行。
    3. Top-K稀疏: 在每个像素点上仅激活前K个专家。
    4. 温度退火: 动态控制分布的锐度。
    """
...
class EnhancedSpatialGate(nn.Module):
    ...
    [优化版] 上下文感知噪声空间门控
    针对 MFIF 任务优化:
    1. 引入空洞卷积: 扩大感受野，更好捕捉聚焦纹理。
    2. 移除 BN 层: 保护对比度信息，改用 GroupNorm 或移除。
    3. SE-Style 全局融合: 全局特征先调制通道，再作为 Bias 叠加。
    4. 稳定噪声逻辑: 简化噪声计算，防止少专家时的数值崩溃。
    """
    def __init__(self, input_size, num_experts, top_k, noise_strength=0.1, use_temperature_annealing=True):
        ...
        mid_channels = input_size // 2
        
        # 【修改点】动态计算 GroupNorm 的组数
        # 我们希望组数是 4，但如果 mid_channels 无法被 4 整除（比如是 9），
        # 就自动降级为 2 或 1 (即 LayerNorm)。
        # math.gcd(4, 9) = 1, math.gcd(4, 16) = 4, math.gcd(4, 6) = 2
        safe_groups = math.gcd(4, mid_channels)

        self.spatial_conv = nn.Sequential(
            # Layer 1: 空洞卷积 (dilation=2)，感受野 5x5
            nn.Conv2d(input_size, mid_channels, kernel_size=3, padding=2, dilation=2, bias=False),
            
            # 使用计算出的安全组数
            nn.GroupNorm(safe_groups, mid_channels), 
            
            nn.LeakyReLU(0.2, inplace=True),
            
            # Layer 2: 标准卷积
            nn.Conv2d(mid_channels, num_experts, kernel_size=1)
        )
...
class AMoFE(nn.Module):
    """
    Spatial-Aware Attribute-aware Mixture of Fusion Experts (v3)
    集成上下文感知噪声空间门控 (CANS-Gate)
    """
