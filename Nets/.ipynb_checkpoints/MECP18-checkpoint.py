...
#         self.feature_extraction = Initial_Conv(dim_in=3, out_channels=18)
        
#         self.gcn_bn1 = nn.GroupNorm(6, 18)
#         self.gcn_bn2 = nn.GroupNorm(6, 36)

#         self.mix = nn.Conv2d(6, 1, kernel_size=7, stride=1, padding=3)

#         # 1. 编码器
#         self.cnn_encoder1 = AELF_Encoder(in_dim=18, out_dim=36, kernel_size=3, act=nn.Mish(inplace=True), bias=True, cross=cross)

#         self.skip_mix_1 = nn.Conv2d(18 * 2, 18, kernel_size=1, bias=True)
#         self.skip_mix_2 = nn.Conv2d(36 * 2, 36, kernel_size=1, bias=True)

#         # 2. Decoder
#         self.cnn_decoder = AELF_Decoder(in_dim=48, out_dim=24, enc_in_dim=18, kernel_size=3, act=nn.Mish(inplace=True), bias=True)
...
#                     out_a = out_a.permute(1, 0).view(1, 18, h_s, w_s)
...
#                     out_b = out_b.permute(1, 0).view(1, 18, h_s, w_s)
...
#             gcn_result_smooth = F.avg_pool2d(gcn_result, kernel_size=3, stride=1, padding=1)
#             # 融合
...
class Network(nn.Module):
    ...
    def forward(self, A, B):
        ...
        if self.use_gcn:
            ...
            max_nodes = 10000 
            base_stride = 2
            min_grid_stride = max(base_stride, int((h * w / max_nodes) ** 0.5) + 1)
